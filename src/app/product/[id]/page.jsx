...
import { 
  ShoppingBag, ArrowLeft, Star, Truck, 
  ShieldCheck, RotateCcw, Calendar, Zap 
} from "lucide-react";
...
export default function ProductDetailsPage() {
  ...
  if (!product) return (
    <div className="min-h-screen flex flex-col items-center justify-center text-[#5D4037] gap-4">
      ...
      <Link href="/product"><Button className="bg-orange-600">Back to Shop</Button></Link>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#FDFBF9] pb-20">
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          ...
          {/* IMAGE SECTION */}
          <div className="relative aspect-square rounded-[3rem] overflow-hidden bg-white shadow-2xl shadow-orange-900/10">
            <Image
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-stone-400 font-medium">Verified Artisan Quality</span>
            </div>
            ...
            <div className="flex items-center gap-8 mb-8">
              ...
              <div>
                <p className="text-xs uppercase tracking-widest text-stone-400 font-bold mb-1">Date Listed</p>
                <div className="flex items-center gap-2 text-[#5D4037] font-bold">
                  <Calendar size={16} />
                  <span>{product.date || "New Arrival"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mb-10">
              <h3 className="text-lg font-bold text-[#5D4037] mb-3 uppercase tracking-wider">Description</h3>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 mb-12">
              <div className="flex items-center justify-between border-2 border-orange-100 rounded-2xl px-4 py-3 bg-white">
                <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="text-2xl w-10 h-10 flex items-center justify-center rounded-xl hover:bg-orange-50 text-[#5D4037]">-</button>
                <span className="px-8 font-black text-xl min-w-[60px] text-center">{quantity}</span>
                <button onClick={() => setQuantity(q => q + 1)} className="text-2xl w-10 h-10 flex items-center justify-center rounded-xl hover:bg-orange-50 text-[#5D4037]">+</button>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 p-6 rounded-[2rem] border-2 border-dashed border-orange-100 bg-orange-50/30">
              <div className="flex flex-col items-center text-center gap-2">
                ...
                <span className="text-[10px] font-bold uppercase text-stone-500">Free Delivery</span>
              </div>
              <div className="flex flex-col items-center text-center gap-2 border-y sm:border-y-0 sm:border-x border-orange-100 py-4 sm:py-0">
                ...
                <span className="text-[10px] font-bold uppercase text-stone-500">2 Year Warranty</span>
              </div>
              <div className="flex flex-col items-center text-center gap-2">
                <RotateCcw className="text-orange-600" size={24} />
                <span className="text-[10px] font-bold uppercase text-stone-500">Ready to Ship</span>
              </div>
            </div>
          </div>
        </div>
